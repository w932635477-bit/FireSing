...
> **已优化为 9:16 竖版**（匹配 Kling 3.0 原生输出）
...
| 优化 | 原因 |
|------|------|
| 9:16 竖版构图 | Kling 3.0 原生支持 9:16，避免裁剪损失 |
| 主体居中偏下 | 留出上方空间给运动（zoom in）和字幕 |
| 避免边缘元素 | Kling 生成时边缘可能变形 |
...
- [ ] 人物场景：自然光，不做作
- [ ] 数据场景：极简设计，不要花哨
- [ ] 边缘干净无重要元素（避免 Kling 变形）
- [ ] 每张图都能单独作为"电影截图"看

---

## Midjourney → Kling 最佳实践

1. **生成 4 张候选**，选构图最干净、光影最立体的 1 张
2. **避免选择**：边缘有人/物的图、文字太多的图、过于对称的图
3. **在 Kling 里**：参考图是第一帧，所以构图必须考虑运动方向
